...
export class Overlay extends Partner {
    ...
    createOverlay(locationImage) {
        ...
        document.querySelector('body>div').setAttribute('class', 'overlay-partner');

        gsap.to('.overlay-partner', .3, {opacity:"1", ease: Power3.easeInOut});
    ...
    async deleteOverlay() {
        ...
        gsap.to('.overlay-partner', .2, {opacity:"0", ease: Power3.easeInOut});

        // Attente de 500 miliseconds
        await sleep(500);

        // On supprime l'overlay
        document.querySelector('.overlay-partner').remove();
